...
### 3.0 Структура в репозитории

Текущий репозиторий (`ambition`) содержит React Native приложение. Весь код нового проекта живёт **исключительно в поддиректории `photo-ai/`** — никак не пересекается с существующим кодом.

```
ambition/
├── app/           ← React Native (не трогаем)
├── components/    ← React Native (не трогаем)
├── ...
├── PHOTO_AI_PLAN.md
└── photo-ai/      ← ВСЁ что относится к AI Photo сервису
    ├── backend/
    ├── frontend/
    └── tests/
```

Для запуска бэкенда: `cd photo-ai && uvicorn backend.main:app`
...
|---|---|---|---|
| **Самозанятый** | 10 минут, 4% налог с физлиц, нет взносов, Робокасса берёт на себя 54-ФЗ | ЮКасса не работает (нужна Робокасса), лимит 2.4 млн/год | **Оптимально для валидации** |
| ИП на УСН 6% | Все платёжки, без лимита | 3-5 дней, ~49,500₽ страховых взносов в год | Переходить когда доход >150k/мес |
| ООО | Доп. доверие для B2B | Сложно, дорого | Не нужно |

**→ Решение: начать как самозанятый, переоформиться в ИП при доходе >150,000₽/месяц стабильно**

Самозанятость для этого проекта работает полностью:
- Регистрация: приложение «Мой налог», 10 минут, бесплатно
- Платёжка: **Робокасса** принимает самозанятых и выступает фискальным агентом (сама выдаёт чеки по 54-ФЗ — онлайн-касса не нужна)
- Налог: 4% с платежей от физлиц (против 6% у ИП + фиксированные взносы)
- При обороте до 2.4 млн/год (200 заказов по 990₽ в месяц) самозанятость выгоднее ИП
- РКН: самозанятый может зарегистрироваться оператором ПДн как физлицо

**Когда переходить на ИП**: >150,000₽/месяц стабильно, или нужна ЮКасса, или B2B клиенты.

**День 2-4: Регистрация самозанятого**
- Скачать приложение «Мой налог» (ФНС России)
- Зарегистрироваться через Госуслуги — 10 минут
- Указать вид деятельности: «Создание изображений с помощью искусственного интеллекта» / «Услуги в сфере информационных технологий»
- Счёт: любая карта физлица (Тинькофф, Сбер) — вывод напрямую на неё
...
**День 14-21: Подключение Робокасса (для самозанятого)**
- Заявка на сайте Робокасса
- Нужно: данные самозанятого (ИНН), сайт с офертой и политикой конфиденциальности
- Срок: 1-2 рабочих дня на верификацию
- Комиссия: ~3.9% (физлица), СБП доступен
- Робокасса = фискальный агент: чеки 54-ФЗ формирует сама, онлайн-касса не нужна

**При переходе на ИП → подключить ЮКасса**
- Срок: 3-7 рабочих дней на верификацию
- Комиссия: 3.5% (физлица), ~2.8% (СБП)
- Нужна будет облачная касса: ОФД Яндекс (~2,500₽/год)
...
- **Blue Ocean (Ким/Моборн)**: позиционировать **против фотографов**, не против других AI-сервисов. Клиент не ищет «лучший AI для аватарок» — он ищет «профессиональное фото» и видит фотографов за 5,000–15,000₽ с записью на 2 недели. Наш стратегический canvas: убрать сессию/позирование/ожидание, радикально снизить цену, сохранить качество вплотную к фотографу. Это и есть value innovation — differentiaton + low cost одновременно.
